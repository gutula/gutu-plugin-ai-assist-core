...
import { businessFlowDefinitions, generateSummary, routeTriageSuggestion, reviewAiAnomaly, placeRecordOnHold, releaseRecordHold, amendRecord, reverseRecord } from "../../src/flows/catalog";
...
describe("plugin manifest", () => {
  it("keeps a stable package id and business contract surface", () => {
    expect(manifest.id).toBe("ai-assist-core");
    expect(manifest.kind).toBe("plugin");
    expect(manifest.publicCommands).toContain("ai-assist.summaries.generate");
    expect(manifest.publicEvents).toContain("ai-assist.summary-generated.v1");
  });
});
...
describe("domain catalog", () => {
  it("keeps ERPNext parity references and operational surfaces visible", () => {
    ...
    expect(typeof routeTriageSuggestion).toBe("function");
    expect(typeof reviewAiAnomaly).toBe("function");
