...
describe("ai-assist-core lifecycle integration", () => {
  const previousStateDir = process.env.GUTU_STATE_DIR;
  let stateDir = "";

  beforeEach(() => {
    stateDir = mkdtempSync(join(tmpdir(), "ai-assist-core-"));
  ...
  it("creates, advances, and reconciles a governed business record", async () => {
    const created = await createPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "ai-assist-core:demo",
      title: "AI Assist Core Demo",
      ...
      correlationId: "ai-assist-core:corr",
      processId: "ai-assist-lifecycle:demo"
    });
    ...
    const reconciled = await reconcilePrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "ai-assist-core:demo",
      exceptionId: "ai-assist-core:exception",
    ...
    const amended = await amendPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "ai-assist-core:demo",
      amendedRecordId: "ai-assist-core:demo:amended",
      expectedRevisionNo: 5,
      title: "AI Assist Core Demo Amendment",
      reasonCode: "commercial-correction"
    });
    expect(amended.amendedRecordId).toBe("ai-assist-core:demo:amended");

    const reversed = await reversePrimaryRecord({
      tenantId: "tenant_demo",
      actorId: "actor_admin",
      recordId: "ai-assist-core:demo:amended",
      reversalRecordId: "ai-assist-core:demo:amended:reversal",
      expectedRevisionNo: 1,
      reasonCode: "cancel-amendment"
    });
    expect(reversed.reversalRecordId).toBe("ai-assist-core:demo:amended:reversal");
