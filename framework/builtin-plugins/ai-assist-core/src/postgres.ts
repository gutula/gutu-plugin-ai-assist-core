export type AiAssistCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildAiAssistCoreMigrationSql(options: AiAssistCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "ai_assist_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getAiAssistCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getAiAssistCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildAiAssistCoreRollbackSql(options: AiAssistCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "ai_assist_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "ai_assist_core";
  return [
...
export function getAiAssistCoreLookupIndexName(): string {
  return "ai_assist_core_primary_lookup_idx";
}

export function getAiAssistCoreStatusIndexName(): string {
  return "ai_assist_core_exception_status_idx";
}
