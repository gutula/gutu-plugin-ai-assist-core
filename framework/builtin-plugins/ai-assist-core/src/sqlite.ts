export type AiAssistCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildAiAssistCoreSqliteMigrationSql(options: AiAssistCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "ai_assist_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getAiAssistCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getAiAssistCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildAiAssistCoreSqliteRollbackSql(options: AiAssistCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "ai_assist_core_");
  return [
...
export function getAiAssistCoreSqliteLookupIndexName(tablePrefix = "ai_assist_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getAiAssistCoreSqliteStatusIndexName(tablePrefix = "ai_assist_core_"): string {
